...
//            throw new OperationFailedException(ResultEnum.USER_SELECT_FAILED);
    ...
    @Override
    public List<User> findByCondiction(UserDto userDto) {
        SimpleDateFormat dateFormat = new SimpleDateFormat("YYYY-MM-dd hh:mm:ss");
        if (!Objects.isNull(userDto.getRegTime())){
            userDto.setReg(dateFormat.format(new Date(System.currentTimeMillis() - userDto.getRegTime() * 1000)));
        }
        if (!Objects.isNull(userDto.getLastLoginTime())){
            userDto.setLastLogin(dateFormat.format(new Date(System.currentTimeMillis()-userDto.getLastLoginTime() * 1000)));
        }
        List<User> list = userMapper.findByCondiction(userDto);
        return list;
    }
}
